...
function getChoices(data, nb) {
	...
	while (i < nb + nbFields) {
		if (data[i].name == `choice`) {
			currentChoice = {};
			goodAnswer = false;
			currentChoice[data[i].name] = htmlEntitiesToText(data[i].value);
...
function getQuizData() {
	...
	for (let i = 0; i < data.questions.length; i++) {
		...
		for (let j = 0; j < currentQuestion.choices.length; j++) {
			...
			$(`#choice${ j + 1 }Q${ i + 1 }`).val(textToHtmlEntities(currentChoices.choice));
...
/**
 * This function is used to transform HTML entities into text.
 *
 * @param str
...
/**
 * This function is used to transform text into HTML entities.
 *
 * @param str
